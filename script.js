// Dead-simple Cybertruck intake with VIN-first flow, progress, Blob uploads, and mailto to contact@onlyev.com
...
const steps=[
  ...
  // Contact
  { key:"contact.mobile", prompt:"Mobile # (we’ll text you)", type:"text", placeholder:"5551234567",
    validate:v=>v.replace(/\D/g,"").length>=10 },
  { key:"contact.email", prompt:"Email (if you prefer email)", type:"text", placeholder:"you@example.com",
    validate:v=>v.trim()==="" || /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v.trim()), required:false },
...
function currentIndexAmongShown(k){
  let idx=0; for(const s of steps){ if(s.type==="info") continue; if(s.when && !s.when()) continue; if(s.key===k) return idx+1; idx++; } return 1;
}
...
function addQAItem(s){
  if(s.type==="info"){ // banner card
    const info=document.createElement("div");
    info.className="qa-item";
    const inner=document.createElement("div");
    inner.className="qa-answer";
    const title=document.createElement("div");
    title.style.fontWeight="600"; title.textContent=s.prompt;
  ...
  if(openDetails) openDetails.open=false; openDetails=details;
...
function renderField(s){
  ...
  if(s.type==="text"){
    ...
    const sel=mk("select",{className:"select"}); for(const opt of s.options) sel.appendChild(mk("option",{value:opt},opt));
    ...
    const file=mk("input",{className:"file",type:"file"}); if(s.accept) file.accept=s.accept; if(s.multiple) file.multiple=true;
...
function nextStep(){
  while(++step<steps.length){
    ...
    // auto-skip info card and immediately open next real input
...
async function uploadToBlob(files){
  const urls=[]; for(const f of files){ const {url}=await getUploadURL(f); await fetch(url,{method:'PUT',body:f}); urls.push(url.split('?')[0]); } return urls;
}
